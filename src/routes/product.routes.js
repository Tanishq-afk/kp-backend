...
// Create (admin). The model additionally validates that each size value belongs
// to the chosen sizeType.
router.post(
  '/',
  restrictTo(ROLE.ADMIN),
  [
    body('name').trim().notEmpty().withMessage('Product name is required'),
    body('articleNumber').trim().notEmpty().withMessage('Article number is required'),
...
// Update details (admin). Sizes/sizeType are not editable here.
router.patch(
  '/:id',
  restrictTo(ROLE.ADMIN),
  [
    ...
    body('articleNumber').optional().trim().notEmpty(),
